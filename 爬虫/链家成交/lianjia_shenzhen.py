    ...
    for url in urls:
        ...
        for details_url in details_urls:
            try:
                ...
                #文件逐行写如csv
                with open('./data_shenzhen.csv','a+',encoding='utf_8_sig',newline='') as f:
                    f_csv = csv.writer(f)
                    f_csv.writerow(data)
                print(data)
...
def get_allurl():
    ...
    req = requests.get('https://sz.lianjia.com/chengjiao/',headers=headers)
    html = etree.HTML(req.text)
    urls = html.xpath('//div[@data-role="ershoufang"]/div[1]/a/@href')
    urls = ['https://sz.lianjia.com'+ url for url in urls]
