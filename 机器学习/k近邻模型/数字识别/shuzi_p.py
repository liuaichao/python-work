...
def draw(x_l, y_l):
    ...
    plt.plot(x_l, y_l, color='black', linewidth=2.0, linestyle='--')
    #设置标题
...
#定义函数
#训练模型
def train(X_data, all, datas_test, name_zhen):
    x_l = [5, 6, 7, 8, 9, 10]
    # 定义图表y轴
    y_l = []
    # 将x轴代表的6个k值进行循环带入
...
if __name__ == '__main__':
    ...
    all = anal(r'.\\trainingDigits')
    #将处理过的训练数据转化成 测试数据文件个数*1024的矩阵
    X_data = np.array(all[1]).reshape(len(listdir(r'.\\trainingDigits')),1024)
    # data_test = np.array(data_test)
    #调用模型训练函数
    draw_data = train(X_data, all, datas_test, name_zhen)
    #调用绘图函数
    draw(draw_data[0], draw_data[1])
